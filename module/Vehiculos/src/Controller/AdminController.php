...
class AdminController extends AbstractActionController
{
    ...
    public function generarLoteAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            error_log("=== GENERAR LOTE: Cantidad solicitada: $cantidad ===");
            ...
            error_log("=== GENERAR LOTE: Códigos generados: " . count($codigos) . " ===");
            error_log("=== GENERAR LOTE: Ejemplo primer código: " . json_encode($codigos[0] ?? null) . " ===");

            // Generar PDF con los QR
            error_log("=== GENERAR LOTE: Antes de llamar generarPdfConQr ===");
            $pdfUrl = $this->generarPdfConQr($codigos);
            error_log("=== GENERAR LOTE: PDF generado en: $pdfUrl ===");
    ...
    private function generarPdfConQr(array $codigos): string
    {
        try {
            ...
            $tempPath = $this->appConfig['temp_path'] ?? __DIR__ . '/../../../../../public/vehiculos/temp';
            $baseUrl = $this->appConfig['base_url'] ?? '/vehiculos/public';
            ...
            error_log("PDF Generator - Iniciando generación para " . count($codigos) . " códigos");
            error_log("PDF Generator - Directorio temp: " . $tempPath);
            error_log("PDF Generator - Base URL: " . $baseUrl);
            ...
            error_log("PDF Generator - Guardando PDF en: " . $filepath);

            $pdf->Output($filepath, 'F');

            if (file_exists($filepath)) {
                error_log("PDF Generator - ✓ PDF generado exitosamente: " . filesize($filepath) . " bytes");
            } else {
                error_log("PDF Generator - ✗ ERROR: El archivo no se generó");
            }

            return $baseUrl . '/vehiculos/temp/' . $filename;
        } catch (\Exception $e) {
