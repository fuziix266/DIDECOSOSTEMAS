<?php
/**
 * Extraer y actualizar datos de trámites de OCAM
 */
...
try {
    ...
    $viewsPath = __DIR__ . '/../view/departamentos/ocam/';
    $files = glob($viewsPath . '*.phtml');
    
    $departamentoId = 3; // Servicios para Personas Mayores (OCAM)
    $tramitesData = [];
    ...
    foreach ($files as $file) {
        ...
        // Extraer documentos requeridos (card-body)
        preg_match('/Documentos Requeridos.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchDoc);
        ...
        // Extraer requisitos
        preg_match('/Requisitos del Usuario.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchReq);
        ...
        preg_match('/Instrucciones Paso a Paso.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchInst);
        ...
        preg_match('/Tiempo Estimado.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchTiempo);
        $tiempo = isset($matchTiempo[1]) ? trim(strip_tags($matchTiempo[1])) : '';
        $tiempo = preg_replace('/\s+/', ' ', $tiempo);
        $tiempo = trim($tiempo);

        // Extraer responsable
        preg_match('/Responsable.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchResp);
        ...
        preg_match('/Observaciones.*?<div class="card-body">(.*?)<\/div>/s', $content, $matchObs);
